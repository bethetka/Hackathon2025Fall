...
let DeleteSessionPublicParams = class DeleteSessionPublicParams {
    sessionId;
};
__decorate([
    String(),
    __metadata("design:type", String)
], DeleteSessionPublicParams.prototype, "sessionId", void 0);
DeleteSessionPublicParams = __decorate([
    Schema()
], DeleteSessionPublicParams);
export { DeleteSessionPublicParams };
export default {
    ...
    async handler(ctx) {
        if (await authenticate(ctx)) {
            ...
            if (!session)
                return MakeError(Errors.SESSION_NOT_FOUND);
            if (session.user.toString() != ctx.meta.user._id.toString())
                return MakeError(Errors.SESSION_NOT_FOUND);
...
//# sourceMappingURL=deleteSessionPublic.js.map